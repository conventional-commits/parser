...
class Scanner {
  ...
  abort (node, expectedTokens = []) {
    const validTokens = expectedTokens.filter(Boolean).join(', ')
    const error = this.eof()
      ? Error(`unexpected token EOF valid tokens [${validTokens}]`)
      : Error(`unexpected token '${this.peek()}' at position ${this.pos.line}:${this.pos.column} valid tokens [${validTokens}]`)
